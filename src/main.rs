...
fn main() {
    ...
    //
    print_difference(coords.0, coords.1); // Rust uses dot notation to index into the tuple
    ...
    //
    ding(series[6]);
    // Unlike tuples arrays can be accessed via direct index accessors
    ...
    //
    on_off(mess.2[1].0);
    // Yikes, mess is an understatement, tuple, of literal array of tuples all with mismatched values
